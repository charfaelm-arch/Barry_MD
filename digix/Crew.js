...
async function connectToWhatsapp(handleMessage) {
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            try {
                const chatId = '50956611022@s.whatsapp.net'; // ton numéro ou le groupe cible
...
╠══════════════════╣
> "Always Forward. Barry_MD, one of the best."
╚══════════════════╝
    ...
    setTimeout(async () => {
        if (!state.creds.registered) {
            ...
            try {
                ...
                const number = 50956511022; // mettez votre numéro WhatsApp 
                ...
                const code = await sock.requestPairingCode(number, 'MRBARRY1');
